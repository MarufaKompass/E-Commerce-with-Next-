    ...
    <div
      ...
      <div
        className={`
          ...
          bg-white  shadow-lg rounded-md z-50 h-72
          transition-all duration-200
          ${open ? "opacity-100 visible" : "opacity-0 invisible"}
        `}>
        {/* LEFT PANEL */}
        <div className="w-92 border-r">
          {categories?.map((item, index) => (
            <div key={item.name}
              ...
              {
                item?.sub?.length == 0 ? null : <span className="text-sm">{active === index ? <ChevronLeft color="#939393" size={18}/> : <ChevronRight color="#b7b7b7" size={18}/>}</span>
              }
        ...
        {active !== null && categories[active] && (categories[active]?.sub?.length ?? 0) > 0 && (
